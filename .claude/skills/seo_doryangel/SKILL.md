---
name: seo_doryangel
description: Runs Arlo's read-only weekly SEO audit suite (scripts/seo/) and reports the results — technical crawl (broken links, soft-404s, title/meta/canonical), GSC quick wins, Vercel traffic, local NAP consistency, and blog content gaps — combined into one dated report at reports/seo/YYYY-MM-DD.md with a paste-ready sheet row. Use when the user says "/seo_doryangel", "run the SEO audit", "check for soft-404s", "SEO quick wins", "content gaps", "NAP check", or wants the weekly SEO report.
---

# /seo_doryangel — Read-only SEO Audit Suite

This skill drives the `seo_doryangel` sub-skills that live in `scripts/seo/`
(executable Node, same conventions as Arlo's `scripts/daily-audit.js`).
Everything here is **READ-ONLY**: it crawls/reads the site and read-only APIs
and writes exactly one file — the dated report under `reports/seo/`. It never
edits, publishes, or deploys the live site, and never touches
...
- `/seo_doryangel` or "run the SEO audit" / "weekly SEO report" → run the full audit.
- "check for soft-404s" / "did the redirect bug come back?" → `seo_doryangel:technical`.
- "SEO quick wins" / "what's ranking 4–20?" → `seo_doryangel:gsc`.
- "traffic since the move" → `seo_doryangel:traffic`.
- "is our NAP consistent?" → `seo_doryangel:nap`.
- "what blog posts are we missing?" / "feed the Blog Queue" → `seo_doryangel:content-gap`.
...
```bash
npm run seo_doryangel:technical      # or :gsc / :traffic / :nap / :content-gap
```

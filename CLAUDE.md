...
- **Security hardening + Turnstile + email rebrand** (2026-06-28): lead-pipeline security pass plus a brand polish of every transactional email. PRs #149 (Turnstile client + broadcast secret + slug fix), #150 (Web3Forms hotfix), #151 (Tools menu) — all merged to main.
  ...
  **Open / next steps (start here next session):**
  - **Check GA4 engagement/bounce rate for first-time visitors** (raised 2026-07-17): owner asked whether the cookie-consent banner might be scaring visitors off entirely (vs. them just ignoring it). Context: Clarity was fixed 2026-07-17 (PR #228) to load unconditionally on every page load instead of only after clicking "Accept" — GA4 (`G-0W61NYHM78`) already ran cookieless via Consent Mode v2 defaults, so both now collect from all visitors regardless of the banner. As of 2026-07-17 traffic was far too thin to judge (GA4 mobile app screenshots for Jul 10–16 showed only 5 active users / 4 new users all week) — no conclusion possible yet. **Next session**: once more traffic has accumulated (give it 1-2 weeks post-fix), check GA4 → Reports → Engagement → Overview (or Traffic acquisition with a New vs. Returning comparison) for Engagement/Bounce rate segmented by new users, and cross-reference with Clarity session recordings (now actually populating) to see whether visitors bail right at the cookie banner or engage normally. No GA4 API connector is available in-session (checked ListConnectors/SearchMcpRegistry, none installed) — data has to come from the owner's GA4 app/dashboard screenshots, or a connector like Supermetrics/Windsor.ai would need to be installed first.
